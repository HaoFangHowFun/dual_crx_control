...
class JointTest(Node):
    def __init__(
        ...
        super().__init__('dual_crx_synchronized_infinite_test')
        ...
        for value in (motion_period, amplitude_deg,
    ...
    def run_test(self):
        ...
        self.get_logger().info(
            'Run time      : infinite (Ctrl+C to stop)'
        )
        self.get_logger().info(
        ...
        # Retain only recent samples so an indefinite run uses bounded memory.
        history_size = max(1, math.ceil(HISTORY_SECONDS * self.rate_hz))
        history = {'time': deque(maxlen=history_size)}
        for ns in self.robot_namespaces:
            history[ns] = {
                'command': deque(maxlen=history_size),
                'state': deque(maxlen=history_size),
            }
        ...
        try:
            ...
            self.get_logger().warning('Interrupted by user; command publishing stopped.')

        self.get_logger().info('Motion stopped.' if rclpy.ok() else 'ROS shutdown.')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            'Run same-phase periodic motion indefinitely on both robots. '
            'Press Ctrl+C to stop; the plot retains the latest 60 seconds.'
        )
    )
    parser.add_argument(
        '--joint',
    ...
    for name in ('period', 'amplitude', 'initial_hold', 'ramp_time', 'rate'):
